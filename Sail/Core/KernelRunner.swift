...
final class KernelRunner {
    ...
    private var tunDNSRestoreNeeded = false   // 本次 helper 运行是否已尝试接管系统 DNS；恢复不能依赖当前设置开关
    ...
    private func startCore(auto: Bool = false) async {
        ...
        appendLogs(["[启动] \(useHelper ? "TUN(root) 模式" : "用户态")"])
        ...
        do {
            if useHelper {
                ...
                guard ok else {
                    if tunDNSRestoreNeeded {
                        _ = await SailHelperClient.restoreDNS()
                        tunDNSRestoreNeeded = false
            ...
            if useHelper, tunDNSRestoreNeeded {
                _ = await SailHelperClient.restoreDNS()
                tunDNSRestoreNeeded = false
                tunOriginalDNSServers.removeAll()
            }
    ...
    private func makeConfig(subscription: Subscription?,
        ...
        dnsServers.append(dnsServer(provider: .system, tag: "system-dns", domainResolver: nil))
    ...
    private func normalizeDNSReferences(_ config: [String: Any]) -> [String: Any] {
        ...
        var referencedServers = Set<String>()
        func collectServers(_ value: Any) {
            if let dict = value as? [String: Any] {
                if let server = dict["server"] as? String {
                    referencedServers.insert(server)
                }
                for child in dict.values { collectServers(child) }
            } else if let array = value as? [Any] {
                for child in array { collectServers(child) }
            }
        }
